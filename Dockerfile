...
WORKDIR /src
COPY ["game_service.csproj", "."]
RUN dotnet restore "./game_service.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./game_service.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./game_service.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "game_service.dll"]